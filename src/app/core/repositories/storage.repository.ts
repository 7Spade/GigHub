...
export class StorageRepository {
  ...
  async uploadFile(bucket: string, path: string, file: File, options?: UploadOptions): Promise<UploadResult> {
    try {
      ...
      const { data: publicUrlData } = this.supabase.client.storage.from(bucket).getPublicUrl(path);
   ...
   * ```
   */
  async downloadFile(bucket: string, path: string, options?: DownloadOptions): Promise<Blob> {
    try {
  ...
  async listFiles(bucket: string, folder?: string, options?: ListOptions): Promise<FileObject[]> {
    try {
      ...
      const { data, error } = await this.supabase.client.storage.from(bucket).list(folder, {
        limit: options?.limit,
