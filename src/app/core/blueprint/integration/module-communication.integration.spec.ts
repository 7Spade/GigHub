...
class PublisherModule implements IBlueprintModule {
  ...
  publishTaskCreated(taskId: string, taskName: string): void {
    ...
    this.context.eventBus.emit('TASK_CREATED', {
      taskId,
      taskName,
      timestamp: Date.now()
    }, this.id);
  ...
  publishTaskUpdated(taskId: string, changes: Record<string, any>): void {
    ...
    this.context.eventBus.emit('TASK_UPDATED', {
      taskId,
      changes,
      timestamp: Date.now()
    }, this.id);
...
describe('Module Communication Integration', () => {
  ...
  describe('Event-Driven Communication', () => {
    ...
    it('should support wildcard event subscriptions', async () => {
      ...
      const taskEvents = loggerModule.loggedEvents.filter(e => 
  ...
  describe('Error Handling', () => {
    it('should isolate errors in event handlers', async () => {
      ...
      class FailingSubscriber implements IBlueprintModule {
        ...
        async start(): Promise<void> { this.status.set(ModuleStatus.STARTED); }
        async ready(): Promise<void> { this.status.set(ModuleStatus.READY); }
        async stop(): Promise<void> { this.status.set(ModuleStatus.STOPPED); }
        async dispose(): Promise<void> { this.status.set(ModuleStatus.DISPOSED); }
      }
