...
| `SUPABASE_PROJECT_REF` | [Supabase Dashboard](https://supabase.com/dashboard) → Your Project → Settings → General → Reference ID | Connect to your Supabase project |
| `SUPABASE_MCP_TOKEN` | [Supabase Dashboard](https://supabase.com/dashboard) → Your Project → Settings → API → service_role key | Authenticate with Supabase |
...
✅ Copilot can use **Supabase MCP** for database operations  
...
"Create a Supabase RLS policy for users table"
→ Should use Supabase MCP tools
```
...
| Supabase fails | Check both `SUPABASE_PROJECT_REF` and `SUPABASE_MCP_TOKEN` |
