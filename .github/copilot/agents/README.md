...
- **功能**:
  ...
  - MCP 工具使用策略
  - 品質標準與安全檢查
  - **整合工具**: context7, supabase, sequential-thinking, filesystem
...
- **功能**:
  ...
  - 自動 API 驗證
  - 審查評分系統
  - **整合工具**: context7, supabase, sequential-thinking, git
...
- **功能**:
  ...
  - Secrets 洩漏檢測
  - 安全性報告
  - **整合工具**: supabase, filesystem, git, sequential-thinking
...
- **功能**:
  ...
  - 資料庫文檔
  - README 生成
  - **整合工具**: context7, filesystem, supabase, playwright
...
- **功能**:
  ...
  - Hotfix 流程
  - 回滾計畫
  - **整合工具**: github, git, filesystem, supabase
...
### HTTP MCP 伺服器 (3 個)
- **context7**: Angular 20, ng-alain 20, ng-zorro-antd 20, Supabase 2.86 最新文檔
- **github**: GitHub Actions, Issues, PRs, 程式碼搜尋
- **supabase**: 資料庫操作, RLS policies, Edge Functions, 分支管理
...
| 任務類型 | 推薦代理 | 主要工具 |
|---------|---------|---------|
| 一般開發 | default | context7, supabase, filesystem |
| 程式碼審查 | review | context7, supabase, sequential-thinking |
| 測試撰寫 | tests | context7, playwright, filesystem |
| 安全檢查 | security | supabase, filesystem, git |
| CI/CD 管理 | ci | github, git, supabase |
| 文檔撰寫 | docs | context7, filesystem, supabase |
...
1. **使用 context7**: 總是在使用 Angular/ng-alain/Supabase API 前查詢最新文檔
2. **使用 sequential-thinking**: 複雜問題先進行結構化分析
3. **使用 supabase.get_advisors**: 資料庫變更後檢查安全性與效能
