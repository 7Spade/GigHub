...
2. **SUPABASE_PROJECT_REF**
   - Purpose: Your Supabase project reference ID
   - How to get: Supabase Dashboard → Settings → General → Reference ID
   - Example format: `zecsbstjqjqoytwgjyct`
   - Used by: `.github/copilot/mcp-servers.yml` (supabase server)

3. **SUPABASE_MCP_TOKEN**
   - Purpose: Authentication token for Supabase MCP server
...
3. Try commands that use MCP tools:
   ...
   - Ask about Supabase operations (uses supabase MCP)
...
- "How do I set up RLS policies in Supabase?" (should access supabase MCP)
