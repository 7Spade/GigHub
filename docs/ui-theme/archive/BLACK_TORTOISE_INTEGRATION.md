# Azure Dragon Theme Integration Guide
# 青龍主題整合指南

## 概述 (Overview)

本指南說明如何在 GigHub 專案中使用青龍主題。青龍主題已完全整合到專案的樣式系統中，無需額外配置即可使用。

This guide explains how to use the Azure Dragon theme in the GigHub project. The Azure Dragon theme is fully integrated into the project's styling system and works out of the box.
...
所有 LESS 變量（如 `@azure-6`、`@jade-4`、`@gradient-dragon-soaring`）現在可在任何 LESS 檔案中使用。

All LESS variables (like `@azure-6`, `@jade-4`, `@gradient-dragon-soaring`) are now available in any LESS file.
...
ng-zorro-antd 配置已更新為使用青龍主題顏色：

ng-zorro-antd configuration has been updated with Azure Dragon theme colors:
...
```html
<!-- 背景顏色 -->
<div class="azure-bg-primary">主要背景</div>
<div class="azure-bg-gradient">漸變背景</div>
<div class="azure-bg-light">淺色背景</div>

<!-- 文字顏色 -->
<span class="azure-text-primary">主要文字</span>
<span class="azure-text-jade">翡翠綠文字</span>
<span class="azure-text-cyan">青綠文字</span>

<!-- 卡片樣式 -->
<div class="azure-card">標準卡片</div>
<div class="azure-card-featured">特色卡片</div>
<div class="azure-card-highlight">高亮卡片</div>
...
@Component({
  ...
  template: `
    <nz-card [nzTitle]="'統計數據'" class="azure-card">
      <div class="azure-text-primary">主要內容</div>
    </nz-card>
  `
})
...
| `@azure-8` | `#0A688B` | 深色強調 |
| `@azure-9` | `#084C5C` | 最深強調 |
| `@azure-10` | `#06303D` | 深色背景 |

### 輔助色 (Secondary Colors)

| 變量名 | 顏色代碼 | 用途 |
|--------|---------|------|
| `@jade-4` | `#14B8A6` | 翡翠綠（成功狀態） |
...
| 變量名 | 效果 | 用途 |
|--------|------|------|
| `@gradient-dragon-soaring` | 青龍藍 → 翡翠綠 | 主要按鈕、英雄區塊 |
| `@gradient-azure-sky` | 青龍藍 → 青綠 → 翡翠綠 | 大型橫幅、頁面背景 |
...
| 變量名 | 效果 | 用途 |
|--------|------|------|
| `@shadow-azure-sm` | 微陰影 | 卡片、按鈕 |
| `@shadow-azure-md` | 標準陰影 | 浮動元素 |
| `@shadow-azure-lg` | 較大陰影 | 彈窗、抽屜 |
| `@shadow-azure-xl` | 超大陰影 | 模態框 |
| `@glow-azure` | 青龍光暈 | 特殊效果 |
| `@glow-jade` | 翡翠光暈 | 特殊效果 |
...
@Component({
  selector: 'app-login',
  template: `
    <div class="login-container azure-bg-gradient-sky">
      <nz-card class="login-card azure-card">
        <h2 class="azure-text-primary">歡迎回來</h2>
...
@Component({
  ...
  template: `
    <div class="stat-card azure-card dragon-pulse">
      <div class="stat-icon azure-bg-gradient">
        <i nz-icon [nzType]="icon"></i>
      </div>
      <div class="stat-content">
        <h3 class="azure-text-primary">{{ value }}</h3>
...
<ul nz-menu nzMode="inline">
  <li nz-menu-item nzSelected>
    <i nz-icon nzType="dashboard" class="azure-text-primary"></i>
    <span>儀表板</span>
  </li>
  <li nz-menu-item>
    <i nz-icon nzType="project" class="azure-text-cyan"></i>
    <span>專案管理</span>
  </li>
  <li nz-menu-item>
    <i nz-icon nzType="team" class="azure-text-jade"></i>
    <span>團隊協作</span>
  </li>
</ul>
```
...
```html
<!-- 使用焦點可見樣式 -->
<button class="azure-focus-visible">可訪問按鈕</button>
...
**解決方案**:
1. 確認 `src/styles/theme.less` 中已導入青龍主題變量
...
**解決方案**:
1. 確認使用正確的變量名稱（如 `@azure-6` 而非 `@azure6`）
2. 檢查是否有其他樣式覆蓋了青龍主題樣式
...
- [青龍主題設計文檔](../../docs/azure-dragon-theme.md)
- [LESS 變量定義](../../docs/azure-dragon-theme-variables.less)
- [視覺參考與代碼示例](../../docs/azure-dragon-theme-examples.md)
