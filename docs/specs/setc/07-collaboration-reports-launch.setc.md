# SETC-07: 協作報表上線 (Collaboration Reports Launch)

## 📋 任務概述
- **優先級**: 🔴 最高
- **預估時間**: 2–4 週
- **依賴**: SETC-04 (日誌系統), SETC-05 (進度儀表板), SETC-06 (品質驗收)
- **狀態**: 📋 規劃

## 🎯 任務目標
將協作報表模組推向生產環境，整合日誌、任務與驗收資料，提供可過濾、可匯出、並符合權限控管的報表功能。

## ✅ 前置條件
- [ ] SETC-04 日誌系統已穩定且可供 API 查詢
- [ ] SETC-05 進度儀表板的關鍵指標 API 已就緒
- [ ] SETC-06 品質驗收規則與檢查點已定義並可被查詢

## 📌 任務清單

### Phase A — 設計（1 週）
- [ ] 定義報表需求（欄位、篩選條件、匯出格式）
- [ ] 設計資料模型、API contract（包含分頁、排序、匯出端點）
- [ ] 權限模型：ACL / RLS 規則草案
- [ ] 性能目標與驗證計畫

### Phase B — 後端實作（1–2 週）
- [ ] 建立報表聚合 API（支援分頁、篩選、欄位選擇）
- [ ] 實作匯出（CSV / Excel）程式路徑與後端快取策略
- [ ] 權限校驗：在 API 層面套用 RLS 或 Service 層 ACL
- [ ] 效能優化（索引、查詢合併、分批查詢）

### Phase C — 前端實作（1 週）
- [ ] 報表頁面 UI：查詢表單、表格、分頁與匯出按鈕
- [ ] 可存取的篩選與欄位選擇 (保存個人偏好為 optional)
- [ ] 顯示導出進度回饋（若後端為非同步匯出）
- [ ] 無障礙檢查（鍵盤 vs 語音存取）

### Phase D — 測試與上線（0.5–1 週）
- [ ] 單元測試 + 整合測試覆蓋核心邏輯
- [ ] Playwright E2E 測試：篩選、匯出、權限測試
- [ ] 性能測試（P95, P99 指標驗證）
- [ ] 部署腳本與回滾方案驗證

## 🧪 驗收標準
- [ ] 報表能在標準測試資料下正確產生並匯出 CSV/Excel
- [ ] 權限測試案例全部通過（無未授權資料暴露）
- [ ] 關鍵指標載入時間 P95 < 2s（在標準測試環境）
- [ ] CI 中的 E2E 測試綁定分支且通過
...
## 🎯 交付物
- API 規格文檔（OpenAPI / contract）
- 前端頁面與匯出功能
- 自動化測試（unit + E2E）
- 上線指南與 Release Notes

## ⏳ 風險與緩解
- 風險：資料量大時查詢效能不足 → 緩解：分頁、預先聚合快取、限制匯出筆數
- 風險：RLS / ACL 規則錯誤導致資料洩漏 → 緩解：建立權限測試矩陣與審核流程
