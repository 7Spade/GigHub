...
| Supabase 整合 | ✅ | Database, Storage, RLS policies |
...
│   │   ├── construction-log.repository.ts         [Supabase data access]
...
**Bucket**: `construction-photos` (private)
- Photo upload path: `construction-logs/{blueprintId}/{logId}/{timestamp}.{ext}`
- RLS policies for upload/view/delete based on blueprint permissions
- Public URL generation for photo display
...
1. **User Authentication**: Uses Supabase auth.uid()
...
✅ **Supabase Integration**: Database, Storage, RLS  
