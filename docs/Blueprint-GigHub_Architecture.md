...
- **Infrastructure**: Backend services (Supabase, Firebase, Storage)
...
graph TB
    ...
    subgraph "Data Tier - Multi-Tenant Database"
        PRIMARY_DB[(Primary DB - Supabase)]
...
|-----------|------------------------|-----------|
| **Database** | Supabase (PostgreSQL) | Already in use, excellent RLS support, real-time capabilities |
| **Authentication** | Firebase Authentication | Already integrated, mature, supports MFA |
| **Storage** | Supabase Storage / Firebase Storage | Integrated with main platform, secure, scalable |
...
| **Database Hosting** | Supabase Cloud | Managed PostgreSQL, backups, scaling |
| **Monitoring** | Sentry (errors) + LogRocket (sessions) | Comprehensive error tracking and session replay |
| **Analytics** | Supabase Analytics + Custom Dashboard | Built-in, customizable |
...
| **Secrets Management** | Supabase Vault / Vercel Environment Variables | Secure, encrypted, access-controlled |
